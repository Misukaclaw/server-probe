...
.stability-wrap{position:absolute;left:18px;right:52px;bottom:12px;display:grid;grid-template-columns:132px minmax(0,1fr);align-items:center;gap:12px;min-width:0}.stability-text{font-size:11px;color:var(--muted);line-height:1.1;white-space:nowrap}.stability-text strong{display:inline-block;color:var(--text);font-size:13px;font-weight:850;font-variant-numeric:tabular-nums;margin-right:6px}.stability-grid{display:grid;grid-auto-rows:10px;gap:2px;align-items:center;min-width:0;overflow:hidden}.stability-cell{height:10px;min-width:2px;border-radius:999px;background:#deded8}.u-ok{background:#16bd78}.u-good{background:#66ce7f}.u-warn{background:#f1b63d}.u-bad{background:#ef4f58}.u-none{background:#e2e2dc}
...
function uptimeClass(r){if(r===null||r===undefined)return'u-none';if(r>=99)return'u-ok';if(r>=95)return'u-good';if(r>=80)return'u-warn';return'u-bad'}
...
function stabilityGrid(st){st=st||{};const hours=Array.isArray(st.hours)?st.hours:[];const pct=st.percent===null||st.percent===undefined?'--':st.percent.toFixed(2)+'%';const days=st.display_days||7;const cells=hours.map(h=>{const text=h.total?`${formatHour(h.ts)} · ${h.ratio.toFixed(2)}% · ${h.ok}/${h.total}`:`${formatHour(h.ts)} · 无数据`;return`<span class="stability-cell ${uptimeClass(h.ratio)}" title="${text}"></span>`}).join('');const cols=Math.max(1,hours.length);return`<div class="stability-wrap"><div class="stability-text"><strong>${pct}</strong>近${days}天稳定性</div><div class="stability-grid" style="grid-template-columns:repeat(${cols},minmax(2px,1fr))">${cells}</div></div>`}
...
function serverCard(s){
  ...
  const stableHtml=stabilityGrid(s.stability);
